#!/usr/bin/env node
/**
 * Test Script: Workout App API Testing
 * 
 * This script tests:
 * 1. Google Generative API connection
 * 2. MongoDB connection and user storage
 * 3. User registration and login
 * 4. Workout plan generation and storage
 * 5. Fetching and updating workouts
 * 
 * Usage: node test-api.js
 * 
 * Make sure your backend is running (npm run dev) before running this!
 */
...
async function testGoogleAPI() {
  ...
  try {
    ...
    if (data.candidates && data.candidates.length && data.candidates[0].output) {
      log('success', 'Google API Connection ✓');
      log('info', 'Sample response:', data.candidates[0].output.substring(0, 100));
      return true;
    } else {
...
async function queryDatabase() {
  ...
  try {
    ...
    const { default: Workout } = await import('./backend/src/models/Workout.js');
